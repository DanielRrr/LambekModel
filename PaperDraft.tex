...
Let us define forcing relation $\Vdash$ between elements of $\mathcal{Q}$ and types as follows for arbitrary valuation map $w : Tp \to \mathcal{A}$:
\begin{enumerate}
  \item Let $p_i \in Tp$, then $\langle a, b \rangle \Vdash p_i \Leftrightarrow \langle a, b \rangle \in w(p_i)$;
  \item $\langle a, b \rangle \Vdash {\bf 1} \Leftrightarrow \langle a, b \rangle \in \mathcal{I}$;
  \item $\langle a, b \rangle \Vdash A \backslash B \Leftrightarrow \forall c \in A, \langle c, a \rangle \Vdash A \Rightarrow \langle c, b \rangle \Vdash A$;
  \item $\langle a, b \rangle \Vdash B / A \Leftrightarrow \forall c \in A, \langle a, c \rangle \Vdash A \Rightarrow \langle b, c \rangle \Vdash A$;
  \item $\langle a, b \rangle \Vdash A \bullet B \Leftrightarrow \exists c, \langle a, c \rangle \Vdash A \text{ and } \langle c, b \rangle \Vdash B$;
  \item $\langle a, b \rangle \Vdash A \lor B \Leftrightarrow \langle a, b \rangle \Vdash A \text{ or } \langle a, b \rangle \Vdash B$;
  \item $\langle a, b \rangle \Vdash A \& B \Leftrightarrow \langle a, b \rangle \Vdash A \text{ and } \langle a, b \rangle \Vdash B$;
  \item $\langle a, b \rangle \Vdash !A \Leftrightarrow \exists S_{\langle a, b \rangle} \forall \langle a', b' \rangle \in S_{\langle a, b \rangle}, \langle a', b' \rangle \Vdash A$
  \item $\langle a, b \rangle \Vdash \Gamma \rightarrow A \Leftrightarrow \langle a,b \rangle \Vdash A_1 \bullet \dots \bullet A_n \Rightarrow  \langle a,b \rangle \vdash A$;
\end{enumerate}

\begin{prop}
$ $

Let $\mathcal{R}$ be any R-model:

\begin{itemize}
  \item $\mathcal{R} \models !A \rightarrow !!A$;
  \item $\mathcal{R} \models !A \rightarrow A$.
\end{itemize}
...
\begin{enumerate}
\item Let us show that $\mathcal{Q} \Vdash !A \rightarrow A$.

Let $\mathcal{Q} \Vdash !A$ and $v$ be a valuation, hence $\langle a, b \rangle \Vdash !A$.
Hence there exists $S_{\langle a, b \rangle} \subseteq \mathcal{Q}$,
such that $\forall \langle a', b' \rangle \in S_{\langle a, b \rangle}, \langle a', b' \rangle \Vdash A$,
but $\langle a, b \rangle \in S_{\langle a, b \rangle}$, so $\langle a, b \rangle \Vdash A$.

\item Let us show that $\mathcal{Q} \models !A \rightarrow !!A$.

Let $\langle a, b \rangle \Vdash !A$. Let us show that $\langle a, b \rangle \Vdash !!A$.

So there exists $S_{\langle a, b \rangle} \subseteq \mathcal{Q}$,
such that $\forall \langle a', b' \rangle \in S_{\langle a, b \rangle}, \langle a', b' \rangle \Vdash A$.

Let $S' \subseteq S_{\langle a, b \rangle}$, then for each $\langle a', b' \rangle \in S', \langle a', b' \rangle \Vdash A$,
so $\forall \langle a', b' \rangle \in S_{\langle a, b \rangle}, \langle a', b' \rangle \models !A$, so  $\langle a, b \rangle \models !!A$.
\end{enumerate}
\end{proof}

\begin{theorem} Soundness

  Let $\mathcal{R}$ be a class of $R$-frames, then $Log(\mathcal{R}) = L^{*}_{\bf 1} A_{\bf S 4}$
\end{theorem}

\begin{proof}
$ $

Modal cases follows from the previous lemma.
\end{proof}

\begin{theorem}
$ $

  Let $W : Fm \to 2^R$, s.t. $W(A) = \{ \langle a, b \rangle \: | \: \mathcal{F}, \langle a, b \rangle \}$, then:
  \begin{enumerate}
    \item For each $A \in Fm$, $W(A)$ is a subrelation of $R$;
    \item
    \begin{enumerate}
      \item $W(A \backslash B) = \bigcup \{ \}$
      \item $W (A / B) = $
      \item $W (A \circ B) = $
      \item $W ({\bf 1}) = $
      \item $W (A \lor B) =$
...
\begin{defin} Algebra generated by $R$-frame.
\end{defin}

\begin{theorem}
$ $

  Any relational quantale is isomorphic to some algebra generated by $R$-frame.
\end{theorem}
...
$ $
  ...
  If $\Gamma \models_{R} A$, then $L^{*}_{\bf 1} A_{\bf S 4} \vdash \Gamma \rightarrow A$.
\end{theorem}

...
\begin{theorem}
  Every unital quantale $\mathcal{Q} = \langle A, \leq, \cdot, \bigvee \rangle$ with quantic conucleus $\Box$ is
  isomorphic to some relation quantale $\hat{\Box}$
\end{theorem}

\begin{proof}
  Consider a relational quantale by Brown and Gurr $\hat{\mathcal{Q}} = \langle \mathcal{R}, \subseteq, \circ, \bigvee \rangle$ where:
  \begin{enumerate}
    \item $\hat{a} = \{ \langle b, c \rangle \: | \: b \cdot a \leq c \}$;
    \item $\widehat{a \cdot b} = \hat{a} \circ \hat{b}$;
    \item $\widehat{\bigvee a} = \bigvee \hat{a}$.
  \end{enumerate}

  Consider $\hat{\Box} \hat{a} = \bigvee $

  \begin{enumerate}
    \item $\hat{\Box} \hat{a} \subseteq \hat{a}$;
    \item $\hat{\Box} \hat{a} \subseteq \hat{\Box} \hat{\Box} \hat{a}$;
    \item $\hat{a} \subseteq \hat{b} \Rightarrow \hat{\Box}\hat{a} \subseteq \hat{\Box}\hat{b}$;
    \item $\hat{\Box}\hat{a} \circ \hat{\Box}\hat{b} \subseteq \Box(\hat{a} \circ \hat{b})$;
    \item $\hat{\Box}\hat{\varepsilon} = \hat{\varepsilon}$.
  \end{enumerate}

  Let us show that $\hat{\Box} \hat {a} = \widehat{\Box a} = \{ \langle b, c \rangle \: | \:  b \cdot \Box a \leq c \}$
\end{proof}


\begin{defin}
  Let $W$ be a set. A Kripke frame is a pair $\mathcal{W} = \langle W, R, I \rangle$,
  where $R$ is a ternary relation on $W$ and $I$ is an unary relation on $W$.
  ...
  \begin{enumerate}
    \item $\mathcal{W}, w \Vdash p_i \Leftrightarrow w \in v(p_i)$;
    \item $\mathcal{W}, w \Vdash {\bf 1} \Leftrightarrow I(w)$;
    \item $\mathcal{W}, w \Vdash A \bullet B \Leftrightarrow \exists u, v \in W, R(u,v,w) \text{ and } \mathcal{W}, u \Vdash A \text{ and } \mathcal{W}, v \Vdash B$;
    \item $\mathcal{W}, w \Vdash A \backslash B \Leftrightarrow \forall u, v \in W, R(u,w,v) \text{ and } \mathcal{W}, u \Vdash A \text{ implies } \mathcal{W}, v \Vdash B$;
    \item $\mathcal{W}, w \Vdash B / A \Leftrightarrow \forall u, v \in W, R(w,u,v) \text{ and } \mathcal{W}, u \Vdash A \text{ implies } \mathcal{W}, v \Vdash B$;
    \item $\mathcal{W}, w \Vdash A \lor B \Leftrightarrow \forall u, v \in W, R(u,v,w) \text{ either } \mathcal{W}, u \Vdash A \text{ or } \mathcal{W}, v \Vdash B$;
    \item $\mathcal{W}, w \Vdash A \land B \Leftrightarrow \mathcal{W}, w \Vdash A \text{ and } \mathcal{W}, w \Vdash A$;
    \item $\mathcal{W}, w \Vdash \Gamma \rightarrow A \Leftrightarrow \mathcal{W}, w \Vdash \bullet \Gamma \text{ implies } \mathcal{W}, w \Vdash A$.
  \end{enumerate}
\end{defin}

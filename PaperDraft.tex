...
Also, we have a multiplicative conjunction or non-commutative tensor. This connective plays the role of concatenation.
...
After that, we introduce inference polymodal inference rules for non-commutative subexponentials. One can apply substructural
...
contraction is PSPACE-complete. These results were obtained by Kanovich, Kuznetsov, Nigam, and Scedrov.
...
$ $
  ...
  $I_1 a_1 \cdot I_2 a_2 \leq I_1 (I_1 a_1) \cdot I_2 (I_2 a_2) \leq I (I_1 a_1) \cdot I (I_2 a_2) \leq I (I_1 a_1 \cdot I_2 a_2)$
\end{proof}
...
Note that centrality and strong square-increasing yield centrality.

\begin{lemma}
$ $

  Let $I$ be an open modality on some unital quantale $\mathcal{Q} = \langle A, \bigvee, \cdot, e \rangle$.
...
$ $
  ...
  then $I : \mathcal{Q} \to \mathcal{Q}$, such that $\Box a = \bigvee \{ s \in S \: | \: s \leq a \}$, is an open modality.
\end{prop}
...
We define a subexponential interpretation in a quite sophisticated manner. Suppose we have a contravariant map from signature
to the set of subquantales. Here, contravariance denotes that the stronger subexponential index (in sense of preorder) maps
to the weaker subquantale. The second one function maps a subquantale to its quantic conucleus according to the previous
proposition. We match the index pursuant to its subset. In other words, if $s \in W$, then the result of subexponential
interpretation is an unital quantic conucleus, etc. More formally:

\begin{defin} Interpretation of subexponential signature

Let $\mathcal{Q}$ be a quantale and $\Sigma = \langle \mathcal{I}, \preceq, \mathcal{W}, \mathcal{C}, \mathcal{E} \rangle$ be
a subexponential signature. Suppose, we have $S : \Sigma \to Sub(\mathcal{Q})$, a contravariant map from this subexponential
signature to the set of subquantales of $\mathcal{Q}$, and map $M : \mathcal{P}(\mathcal{Q}) \to \Box_{\mathcal{Q}}$, where
$\Box_{\mathcal{Q}}$ is the set of quantic conuclei on $\mathcal{Q}$. Thus, a subexponential interpretation is the map
$\sigma = M \circ S$ such that:
...
An interpretation and an entailment relation are defined standardly via valuation map and subexponential interpretation. There occur soundness and completeness theorems:
...
By IH, $\sigma(s_1)[\![A_1]\!] \cdot \dots \cdot \sigma(s_n) [\![A_n]\!] \leq [\![A]\!]$. Thus, $\sigma(s)(\sigma(s_1)[\![A_1]\!] \cdot \dots \cdot \sigma(s_n) [\![A_n]\!]) \leq \sigma(s)([\![A]\!])$.

By Lemma, $\sigma(s_1)[\![A_1]\!] \cdot \dots \cdot \sigma(s_n) [\![A_n]\!] \leq \sigma(s)(\sigma(s_1)[\![A_1]\!] \cdot \dots \cdot \sigma(s_n) [\![A_n]\!])$.
...
\begin{proof}
  ...
  On the other hand, $I(a) \leq a$. So $I(a) \leq \bigvee \{ s \in \mathcal{Q}_{I} \: | \: s \leq a \} = I'(a)$.
...
for advice, critique, and comments.
